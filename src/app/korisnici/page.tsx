...
import { Search, SlidersHorizontal, X, Factory, Truck, MapPin, ShieldCheck, Package, Users } from 'lucide-react'
...
function UserCard({ user }: { user: any }) {
  ...
  return (
    <Link
      href={`/profil/${user.id}`}
      className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg transition-all duration-300 p-6 hover:-translate-y-1"
    >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <span className="text-xs text-gray-400">
              {user._count.listings} {user._count.listings === 1 ? 'oglas' : 'oglasa'}
            </span>
          </div>
          {user.bio && (
...
function UsersContent() {
  ...
  const hasFilters = city || userType || searchParams.get('search')
      ...
      <div className="space-y-4">
        <form onSubmit={handleSearch} className="flex gap-2">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Pretražite po nazivu firme, gradu ili PIB-u..."
              value={search}
        ...
        {showFilters && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 bg-white border border-gray-200 rounded-2xl">
            <div>
            ...
            <div>
              ...
              >
                ...
                <option value="GENERATOR">Generatori otpada</option>
                <option value="COLLECTOR">Sakupljači otpada</option>
              </select>
            </div>
          </div>
        )}
          ...
          {pages > 1 && (
            <div className="flex justify-center gap-2 mt-10">
              {Array.from({ length: pages }, (_, i) => i + 1).map((p) => (
                <a
                  key={p}
                  href={`/korisnici?${new URLSearchParams({ ...Object.fromEntries(searchParams.entries()), page: p.toString() }).toString()}`}
                  className={`w-10 h-10 flex items-center justify-center rounded-xl text-sm font-medium transition-colors ${
                    p === page ? 'bg-primary-600 text-white' : 'bg-white border border-gray-300 text-gray-600 hover:bg-gray-50'
                  }`}
                >
                  {p}
                </a>
